<?php 
 	 $textConfig = ["消息推送" =>"Message push",
"安卓设备在首次使用时会提示多种权限设置，请给予LinkTo所有权限，权限的缺失又可能导致手机结束应用进程，从而导致部分数据不完整。" =>"Android devices will prompt for multiple permission settings when they are used for the first time. Please give LinkTo all permissions. The lack of permissions may cause the mobile phone to end the application process, resulting in some data incomplete.",
"第一次使用设备时没有给予消息推送的权限，之后再给予权限，可能导致消息不能正常推送，出现此类情况时，请将设备与应用断开连接然后再次连接，消息就能够正常提示。" =>"The first time you use the device, you don't give permission to push the message. After giving permission, the message may not be pushed properly. When such a situation occurs, disconnect the device from the application and connect again, and the message will be prompted.",
"扫码无法识别" =>"Scan code is not recognized",
...
"手机来电为什么手表没有提醒" =>"Why are there no reminders on the phone?",
...
"怎么选择手表接收指定应用提醒" =>"How to choose a watch to receive a specific app reminder",
...
"为什么切换手机，手环／手表数据没有同步" =>"Why switch the phone, the bracelet/watch data is not synchronized",
...
"连接正常的情况下，手环／手表已经将最新的数据同步到上一台手机后，将不会再有数据同步到新的手机，只需登录同一个账号，所有数据就会同步过来。" =>"When the connection is normal, the bracelet/watch has already synchronized the latest data to the previous phone, and no more data will be synced to the new phone. Just log in to the same account and all data will be synchronized.",
